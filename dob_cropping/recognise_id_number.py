...
def get_digit_contours(thresh, contours, digit_type):
    ...
    # Get the num_digits contours which have the largest height
    desc_cnt_idx = np.flipud(np.argsort(np.array(heights)))
    if digit_type == 'SHORT_ID':
        num_digits = 9
    elif digit_type == 'LONG_ID':
        num_digits = 12
    elif digit_type == 'DD':
        num_digits = 2
    elif digit_type == 'YYYY':
        num_digits = 4
...
def idToStr(img, net_prototxt, caffemodel, digit_type, useGPU=True):
    # Loading image and finding countours
    # img = cv2.imread(filepath, 0)
    _, thresh = cv2.threshold(img, 127, 255, 0)
    _, contours, _ = cv2.findContours(thresh, mode=cv2.RETR_EXTERNAL, method=2)

    num_digits, digit_contours = get_digit_contours(thresh, contours, digit_type)
...
if __name__ == '__main__':
    ...
    print(idToStr(id_only_img, sys.argv[2], sys.argv[3], 'SHORT_ID'))
    plt.imshow(crop_id(sys.argv[1]), cmap = 'gray')
    plt.show()