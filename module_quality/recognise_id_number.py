...
def get_digit_contours(thresh, contours):
    if type(thresh) != type(None) and type(contours) != type(None):
...
def merge_rects(rects, id_groups):
    if rects != None and id_groups != None:
...
def crop_id(image_path, num_top_contours=12, threshold_dist=12):
    ...
    if num_contours < num_top_contours:
        num_top_contours = num_contours
...
if __name__ == '__main__':
    ...
    plt.show()
    print('ID:', idToStr(id_only_img, sys.argv[2], sys.argv[3]))
    plt.imshow(id_only_img, cmap = 'gray')
    plt.show()